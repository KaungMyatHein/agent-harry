...
| **Golden path** | optional | The ideal route to the goal — defines success and feeds path-efficiency. **The user's own example is authoritative — never invent a golden path when the user has given one.** Four ways, in priority of fidelity to user intent: **(1) target example image** — the user attaches a screenshot of the success/end state (this is the source tool's `targetSuccessImage`, ported Claude-native). See the target-image loop below. **(2) user-listed ideal steps → reference pass** — the user lists the happy-path steps and you run ONE guided "reference pass" following them, recording the golden step count + end state (confirm at the Stop Gate before the unguided runs). **(3) end-state URL/route** — a success URL/route when only the destination matters. **(4) derive from the PRD** — if a v4.3 PRD `primary_journey` exists, use its step sequence (the only no-user-input method; use it as a fallback, not over a supplied example). No golden path supplied → skip path-efficiency, report every other metric. |
...
**Target-image golden path — how the loop uses the user's example (method 1).** This is the source tool's `targetSuccessImage`, re-implemented with Claude's own vision (no Gemini). The user attaches a screenshot of the success/end state. Then, on every step, you compare the **current screen** against that **target image**:
