...
6.4. Copy `templates/widgets/*` → `<project>/widgets/` (overwriting; create folder if missing) — v5.3 inline-widget templates (one per decisionData shape: insights/table/callout/metrics) + the agent-specific supplementals `ia-tree` and `wireframe` (v5.4–v5.5 — lo-fi grayscale layouts + section detail loop to component composition). Canonical and rarely edited locally, so overwrite like the agent files. If a pre-v5.3 install has no `widgets/` folder, this creates it; the wildcard copy picks up `wireframe.widget.html` automatically on any refresh.
