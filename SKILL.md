...
7.6. Copy `templates/widgets/` → `<project>/widgets/` (v5.3: inline-widget templates for decisionData rendering — one per shape: `insights`, `table`, `callout`, `metrics`; the orchestrator reads these only when an inline-widget tool like `show_widget` is available, else markdown render)
...
6.4. Copy `templates/widgets/*` → `<project>/widgets/` (overwriting; create folder if missing) — v5.3 inline-widget templates (one per decisionData shape: insights/table/callout/metrics). Canonical and rarely edited locally, so overwrite like the agent files. If a pre-v5.3 install has no `widgets/` folder, this creates it.
